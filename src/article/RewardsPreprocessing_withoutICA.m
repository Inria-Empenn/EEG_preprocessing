...
dirdata='../../data/';
 for participant = 1:500 %Cycle through participants
     ...
     %Load Data
     EEG = []; %Clear past data
     [EEG] = doLoadBVData([dirdata filenames(participant).name]  ); %Load raw EEG data
...
save('../../results/Ref_All_ERP', 'All_ERP'); %Save ERP Data
save('../../results/Ref_All_trials', 'All_trials'); %Save ERP Data
save('../../results/Ref_All_rejChan', 'All_rejChan'); %Save ERP Data

%% RewP_Waveforms
csvwrite('../../results/Ref_RewP_Waveforms.csv',[(-200:2:998)',nanmean(squeeze(All_ERP(26,:,1,:)),2),nanmean(squeeze(All_ERP(26,:,2,:)),2),nanmean(squeeze(All_ERP(26,:,1,:)),2)-nanmean(squeeze(All_ERP(26,:,2,:)),2)]); %Export data. Conditions: Time, Loss, Win, Difference. Electrode 26 is FCz.
%% RewP_Waveforms_AllPs
win_erp=squeeze(All_ERP(26,:,1,:));
loss_erp=squeeze(All_ERP(26,:,2,:));

csvwrite('../../results/Ref_RewP_Waveforms_AllPs.csv',[win_erp,loss_erp]'); %Export data. Conditions: Loss, Win. Electrode 26 is FCz.
%% RewP_Latency
...
csvwrite('../../results/Ref_RewP_Latency.csv',peak_loc'); %Export data
