...
tt1=squeeze(All_ERP2(26,:,1,:));
tt2=squeeze(All_ERP2(26,:,2,:));
...
for condition=1:2
    ...
    for channel=1:nbchan
        for t=1:nbpt
            Presults(channel,t)=ranksum(squeeze(All_ERP(channel,t,condition,:)),squeeze(All_ERP2(channel,t,condition,:)));
...
Moyenne2_gain=nanmean(squeeze(All_ERP2(channelOfInterest,:,1,:)),2);
Moyenne1_loss=nanmean(squeeze(All_ERP(channelOfInterest,:,2,:)),2);
Moyenne2_loss=nanmean(squeeze(All_ERP2(channelOfInterest,:,2,:)),2);
subplot(2,3,4)
...
for channel=1:nbchan
        for t=1:nbpt
            Presults(channel,t)=ranksum(squeeze(All_ERP(channel,t,1,:)-All_ERP(channel,t,2,:)),squeeze(All_ERP2(channel,t,1,:)-All_ERP2(channel,t,2,:)));
        if(Presults(channel,t)<P_Bonf)
            StatDiff(channel,t)=1;
        end
        end
end
diff_btw(1,2)=length(find(StatDiff));
...
diff2=nanmean(squeeze(All_ERP2(channelOfInterest,:,1,:)-All_ERP2(channelOfInterest,:,2,:)),2);
subplot(2,3,6)
