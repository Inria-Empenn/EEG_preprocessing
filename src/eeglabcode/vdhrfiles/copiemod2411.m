...
% dossiersauv='/home/StageEEGpre/data/Raw Data Part 1';
%cd(uigetdir);
cd('/home/StageEEGpre/data/Raw Data Part 1'); %Find and change working folder to raw EEG data
...
cd('/home/StageEEGpre/data/Raw Data Part 1'); %Find and change working folder to saved data from last for loop
...
for participant = 1:length(filenames) %Cycle through participants
   ...
   %Determine a level of rejection for each individual
     ...
     p_chanreject{participant,2} = cellstr(num2str(numbers(rejection_level))); %insert lowest level within which less than ten electrodes have been removed
     p_chanreject{participant,3} = cellstr(num2str(find(trials_removed>numbers(rejection_level)==1))); %Determine the indices of electrodes to be removed
end

%Save channels to reject information into a mat file
save('Chans_rejected_auto','p_chanreject');
...
for participant = 1:length(filenames) %Cycle through participants
    ...
    %Re-Reference
    load('ChanlocsMaster.mat'); %Load channel location file
    chanlocsMaster([10,21]) = []; %Remove channels TP9 and TP10 as they will be the references
