...
#Figure 2 Error Bars  73=nbparticipants 200 le double car premier event puis deuxieme

for (counter in 1:600){ #Cycle through time
  t_data = pdata[,c(counter,601,602)] #Extract current time point
  data[counter,5] = qt(0.975,nbparticipants-1) * (sd(t_data[1:73,1])/sqrt(73)) #Determine 95% confidence interval for condition 1
  data[counter,6] = qt(0.975,nbparticipants-1) * (sd(t_data[74:146,1])/sqrt(73)) #Determine 95% confidence interval for condition 2
  data[counter,7] = qt(0.975,nbparticipants-1) * (sd(t_data[1:73,1]-t_data[74:146,1])/sqrt(73)) #Determine 95% confidence interval for difference
}
...
#### Table 1     
#### Figure 3     
pdata[1:73,1:600] = pdata[1:73,1:600]-pdata[74:146,1:600] #Create difference waves
pdata = pdata[1:73,1:600] #Remove non-difference wave data
...
gain_peaks = rowMeans(pdata[1:73,sum(peak_time_gain,-23):sum(peak_time_gain,23)]) #Determine mean peak values around grand average peak for gain waveform 239=max amplitude gain
lose_peaks = rowMeans(pdata[74:146,sum(peak_time_lose,-23):sum(peak_time_lose,23)]) #Determine mean peak values around grand average peak for loss waveform 246 = max lose 
